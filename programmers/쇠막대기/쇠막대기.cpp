...
int solution( string arrangement )
{
    ...
    {
        if( i + 1 < arrangement.size() && arrangement.substr( i, 2 ) == "()" )
        {
            q.emplace( arrangement.substr( i, 2 ) );    // 레이저 큐 삽입
            ++i;
        }
        else
            q.emplace( arrangement.substr( i, 1 ) );    // 막대기 시작/끝 큐 삽입
    }
    ...
    do
    {
        string str = q.front();
        if( str == "()" )
            answer += stick_cnt;    // 레이저 만나면 누적된 막대기 카운트 합산
        if( str == "(" )
        {
            ++answer;       // 막대기 시작은 항상 +1 조각
            ++stick_cnt;    // 막대기 카운트 +1
        }
        if( str == ")" )
            --stick_cnt;    // 막대기 끝을 만나면 막대기 카운트 -1
        q.pop();            // 이미 처리한 데이터 빼내기
    } while( q.size() );
