...
{
    int answer = 0;
    stack<int> basket;  // 인형 담을 바구니
    for( size_t i = 0; i < moves.size(); ++i )
    {
        int target = moves[ i ] - 1;    // 인형을 집을 타겟 위치의 열 인덱스
        for( size_t j = 0; j < board.size(); ++j )
        {
            int doll = board[ j ][ target ];    // 타겟 인형
            if( doll == 0 )
                continue;   // 0이면 빈칸이니 아래 칸으로 계속 진행

            board[ j ][ target ] = 0;
            if( !basket.empty() && doll == basket.top() )
            {   // 바구니에 인형이 있고, top의 인형이 현재 타겟 인형과 일치하면(같은 인형이 쌓이면 그 두개는 터진다)
                answer += 2;    // 사라지는 카운트 2개 누적
                basket.pop();   // 사라지는 인형 처리
                break;
            }
            basket.emplace( doll ); // 바구니에 타겟 인형 담음
            break;
        }
    }
...
{
    int ret = 0;
    ret = solution( { {0, 0, 0, 0, 0}, { 0, 0, 1, 0, 3 }, { 0, 2, 5, 0, 1 }, { 4, 2, 4, 4, 2 }, { 3, 5, 1, 3, 1 } }, 
                    { 1, 5, 3, 5, 1, 2, 1, 4 } ); // 4
}
