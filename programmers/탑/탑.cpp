...
vector<int> solution( vector<int> heights )
{
    ...
    for (int i = heights.size()-1; i > 0; --i)
    {
        for (int j = i - 1; j >= 0; --j)
        {
            ...
            {
                finded.push_back( j + 1 );  // i보다 큰값 찾으면 해당 순번 담기
                break;
            }
            else if (j == 0)
            {
                finded.push_back( 0 );  // 끝까지 못찾은 경우 0 담기
                break;
            }
        }
    }
    if(finded.size() < heights.size() )
        finded.push_back( 0 );
    reverse( finded.begin(), finded.end() );    // 역순
...
{
    vector<int> ret;
    ret = solution( { 6,9,5,7,4 } );        //[0,0,2,2,4]
    ret = solution( { 3,9,9,3,5,7,2 } );    //[0,0,0,3,3,3,6]
    ret = solution( { 1,5,3,6,7,6,5 } );    //[0,0,2,0,0,5,6]
}
